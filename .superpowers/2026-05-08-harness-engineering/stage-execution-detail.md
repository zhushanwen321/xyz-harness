...
**L1 适用于**：①③⑤⑦⑧⑨（可程序化验证的阶段）
**L1 不适用于**：②④⑥⑩⑪（需要判断力的阶段，只有 L2 subagent 检查）
...
运行：`gate-script.sh 01 .superpowers/{主题}/spec.md .superpowers/{主题}/plan.md`
- 检查 spec.md 存在且非空
- 检查 plan.md 存在且非空
- 检查 plan.md 包含至少 1 个 "Task" 标题
- 通过 → 生成 .xyz-harness/gate/stage-01.pass
...
2. 读取项目文件结构（ls、关键目录），理解项目现状
3. 执行 brainstorming skill：分析需求 → 产出 spec.md
   - 不做交互式提问（subagent 无法与用户交互）
   - 基于需求描述和项目上下文直接产出
   - 不明确的地方在 spec 中标记为 `[待决议]`
4. 执行 writing-plans skill：基于 spec → 产出 plan.md
   - 文件结构规划 → 任务拆分 → bite-sized step
...
**检查项：**
1. `spec.md` 存在且非空
2. `spec.md` 包含必要章节：目标、方案、影响范围、验收标准
3. `spec.md` 无残留 `[待决议]` 标记（或标记数量 ≤ 3 且已标注为低风险）
4. `plan.md` 存在且非空
5. `plan.md` 包含任务拆分（至少 1 个 task）
6. `summary.md` 存在且阶段 ① 状态为"进行中"

**返回：** pass → 继续；fail(回退①, 原因) → 重新派遣执行 subagent
...
**流转规则：**
- 确认 → 进入阶段 ②
- 有修改意见 → 将用户意见作为输入，重新派遣执行 subagent 修改 spec/plan
...
**返回：** pass → 继续；fail(回退①, MUST FIX 未解决) → 主 agent 将 fail.reason 传给执行 subagent，重新派遣修改 spec/plan
...
**返回：** pass → 继续；fail(回退③, 具体失败项) → 重新派遣执行 subagent 修复
...
**返回：** pass → 继续；fail(回退③, MUST FIX 列表) → 主 agent 将 fail.reason 传给执行 subagent，重新派遣编码 subagent 修复
...
**返回：** pass → 继续；fail(回退⑤) → 修复测试；fail(回退③, 代码不可测试) → 回退到编码实现重构
...
**返回：** pass → 继续；fail(回退⑤, MUST FIX 列表) → 重新派遣测试编写 subagent 修复
...
**返回：**
- pass → 继续
- fail(回退⑤, 测试数=0) → 测试未实际运行
- fail(回退③, 编译错误) → 编译问题
- fail(回退③或⑤, 测试失败) → 按错误类型判断
...
**返回：** pass → 继续；fail(回退③, 部署失败原因) → 代码问题回退编码；fail(配置修复) → 修复配置后重试
...
```
⑪ 复盘完成。报告：changes/retrospective.md
