...
#### Step 3: 逐个 plan Task 执行（核心循环）

对每个 plan Task，根据类型展开为不同的 subtask 链：

**后端 task**（task 不涉及 UI 组件/页面/布局/样式）→ 三步 subtask：

```
todolist expand_step(stepId=currentTaskId, subtasks=[
  "TDD 测试编写（harness-tdd-coder）",
  "代码实现（harness-executor）",
  "Spec 合规检查（harness-reviewer）"
])
```

| subtask 顺序 | subagent | 输入 | 产出 | 完成条件 |
|-------------|---------|------|------|----------|
| 1. TDD 测试编写 | `harness-tdd-coder` | 当前 Task 描述 + spec.md 验收标准 | 失败测试文件（`.test.` / `.spec.`） | 测试文件已写入，`todolist complete_subtask(stepId, subtaskId)` |
| 2. 代码实现 | `harness-executor` | 当前 Task 描述 + TDD 测试文件 | 实现代码，测试通过 | 所有 TDD 测试通过，`todolist complete_subtask(stepId, subtaskId)` |
| 3. Spec 合规检查 | `harness-reviewer` | 当前 Task 描述 + spec.md + 实现代码 | 合规报告 | 检查完成，`todolist complete_subtask(stepId, subtaskId)` |

**前端 task**（task 涉及 UI 组件/页面/布局/样式）→ 两步 subtask：

```
todolist expand_step(stepId=currentTaskId, subtasks=[
  "前端实现（harness-frontend-developer）",
  "Spec 合规检查（harness-reviewer）"
])
```
...
| subtask 顺序 | subagent | 输入 | 产出 | 完成条件 |
|-------------|---------|------|------|----------|
| 1. 前端实现 | `harness-frontend-developer` | 当前 Task 描述 + spec.md UI 规格 | 骨架→功能→美化三阶段完成 | UI 组件已实现，`todolist complete_subtask(stepId, subtaskId)` |
| 2. Spec 合规检查 | `harness-reviewer` | 当前 Task 描述 + spec.md + 实现代码 | 合规报告 | 检查完成，`todolist complete_subtask(stepId, subtaskId)` |

> ⚠️ **subtask 执行顺序是强制的：**
> - 每个 subtask 必须在 `todolist complete_subtask` 之后才能开始下一个 subtask
> - **没有 harness-tdd-coder 的产出（失败测试文件），禁止派遣 harness-executor**
> - 前端 task 虽然跳过 TDD，但 spec 合规检查不可跳过

**单个 plan Task 的完整执行伪代码：**
```
for each task in planTasks:
  # 判断类型
  if isFrontendTask(task):
  todolist expand_step(stepId=task.id, subtasks=[
    "前端实现（harness-frontend-developer）",
    "Spec 合规检查（harness-reviewer）"
  ])
  else:
  ...
  # 派遣 subagent 逐个执行 subtask
  for each subtask in task.subtasks:
  dispatch_subagent(subtask.subagent, context={task, spec, testFiles?})
  todolist complete_subtask(stepId=task.id, subtaskId=subtask.id)

  # 全部 subtask 完成
  git commit
  todolist complete_task(taskId=task.id, summary="关键决策和提醒")
```
