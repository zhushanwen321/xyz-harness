...
| 前端+整体评审 | harness-reviewer | xyz-harness-expert-reviewer(计划评审模式) | spec.md + plan.md + plan-frontend.md + 项目根目录 |

两个评审并行执行。主 agent 收集结果后：
- 汇总所有 MUST FIX 到统一的评审报告中
- 后端评审报告写入 `changes/reviews/backend_plan_review_v{N}.md`
- 前端+整体评审报告写入 `changes/reviews/plan_review_v{N}.md`
