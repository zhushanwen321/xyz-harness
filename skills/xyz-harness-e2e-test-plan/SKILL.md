...
5. 如果涉及 UI，必须包含 DOM/A11y 验证（Layer 2），且优先使用 Accessibility Tree（`role + name`）定位元素，而非 CSS class selector。禁止使用 Tailwind 工具类作为 selector。只有 A11y Tree 无法精确匹配时才降级用 `[data-testid]` 或元素文本
...
### L2 验证优先使用 Accessibility Tree

Layer 2 的 DOM/A11y 验证**必须优先使用 Accessibility Tree**，而非 CSS selector 或 class name。原因：A11y Tree 是浏览器计算的真实语义树，与视觉渲染一致，且不受 CSS 重构影响。

- 用 `role + name` 定位元素（如 `role=button, name=保存`），而非 CSS class selector
- **禁止使用 Tailwind 工具类作为 selector**（样式重构即失效，如 `.bg-blue-500`、`.flex.items-center`）
- 只有 A11y Tree 无法精确匹配时才降级用 `[data-testid]` 或元素文本
- 每个用例的“验证方法”列应优先写 A11y 查询方式

**优先级链**：`role + name` > `aria-label` / `alt` / `title` > `[data-testid]` > 元素文本 > CSS class（禁用 Tailwind 工具类）

**降级条件**（满足任一才允许降级到 `[data-testid]` 或元素文本）：
1. 同一页面存在多个相同 `role + name` 的元素（如多个“删除”按钮），且无法通过父容器区分
2. 动态文本导致 name 不确定（如列表项包含变量内容）
3. A11y Tree 中该元素的 role 为 `generic`，无法语义定位
...
### 分组列表
| 组号 | 组名 | 用例数 | 验证层级 | 涉及页面/API |
|------|------|--------|---------|-------------|
| EG1 | {组名} | {N} | L1+L2+L3 | {页面/API 路径} |
| EG2 | {组名} | {N} | L1+L4 | {API 路径} |
| EG3 | {组名} | {N} | L1+L2+L4 | {页面/API 路径} |
...
| Agent | `harness-e2e-tester` |
| Model | `llm-simple-router/glm-5.1` |
| 注入上下文 | e2e-test-plan.md EG1 用例 + spec 相关验收标准 |
| 读取文件 | e2e-test-plan.md（本文件） |
| 写入文件 | `.xyz-harness/{topicDir}/evidence/e2e-test-report.md`（追加） |
| Chrome 实例 | 独立端口 {port}，独立 `--user-data-dir=/tmp/chrome-e2e-eg1` |
| 测试数据 | {按第二章准备，说明本组负责什么数据} |

**用例列表:**

| TC 编号 | 用例名 | 验证层级 | 前置 TC |
|---------|--------|---------|--------|
| TC-1-01 | ... | L1+L4 | 无 |
...
**规则：E2E 测试组严格串行执行。** 原因：共享 Chrome 实例和数据库状态，并行不安全。

| 执行顺序 | 测试组 | 前置依赖 | 说明 |
|---------|--------|---------|------|
| 1 | EG1 | 无 | 基础功能 |
| 2 | EG2 | EG1 | 需要 EG1 创建的数据 |
| 3 | EG3 | EG1 | 需要 EG1 的用户数据 |

**串行约束:**
- 每个测试组由一个 `harness-e2e-tester` subagent 执行
- 前一个组完成后（PASS），才启动下一个组
- 前一个组 FAIL → 记录失败，继续执行后续无依赖的组（依赖该组的跳过）
- 共享同一个 Chrome 实例（同一个 subagent 串行使用，无需多实例隔离）
```
...
**元素定位优先级**：Accessibility Tree (`role + name`) > `[data-testid]` > 元素文本 > CSS class（禁用 Tailwind 工具类）。详见上方“L2 验证优先使用 Accessibility Tree”章节。

#### 2.1 元素存在性检查

**首选：通过 Accessibility Tree 查询元素**

```bash
# 按 role + name 查找目标元素（推荐方式）
...
for n in raw.get('result',{}).get('nodes',[]):
    role = n.get('role',{}).get('value','')
    name = n.get('name',{}).get('value','')
    if role == 'button' and '保存' in (name or ''):
        print(f'FOUND: {n[\"nodeId\"]} role={role} name={repr(name)}')
        found = True
if not found: print('NOT FOUND: role=button with name containing 保存')
"
```

**降级：通过 data-testid 查找元素**（仅在 A11y Tree 无法精确匹配时使用）

```bash
# 通过 data-testid 查找（降级方式）
node scripts/cdp.js "$WS_URL" Runtime.evaluate \
  '{"returnByValue":true,"expression":"!!document.querySelector(\"[data-testid=submit-btn]\")"}'
# 返回 true/false
```
