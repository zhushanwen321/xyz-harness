...
description: >
  需求开发全流程编排器(15 stage)。主 agent 是纯调度器,不直接执行任何业务逻辑。
...
| 文件 | 用途 | 何时读取 |
|------|------|----------|
| `references/glossary.md` | 术语表（中英对照 + 含义） | 前置检查通过后、Stage 1 开始前，主 agent 读取后统一业务语言 |
| `references/claude-md-template.md` | CLAUDE.md 填空模板 | 前置检查发现项目没有 CLAUDE.md 时 |
| `references/phase2-launch-template.md` | Phase 2 启动提示词标准模板 | 二阶段模式下，Step 8 用户确认后 |
...
| Spec 评审 subagent | harness-spec-reviewer | read, bash | glm-5.1 |
| E2E 测试计划评审 subagent | harness-e2e-test-plan-reviewer | read, bash | glm-5.1 |
| 门禁 subagent | harness-gate-checker | read, bash | glm-5.1 |
| 后端设计规划 | harness-backend-planner | read, bash, write | glm-5.1 |
| 后端设计评审 | harness-backend-plan-reviewer | read, bash | glm-5.1 |
...
**Pi** — 将 `extensions/todolist/index.ts` 复制到 `~/.pi/agent/extensions/todolist/`:
```bash
mkdir -p ~/.pi/agent/extensions/todolist
cp extensions/todolist/index.ts ~/.pi/agent/extensions/todolist/
```

- `/track <需求描述>` — 启动 Phase 1（需求沟通，8 步固定流程：讨论→Spec→Spec评审→Plan→Plan评审→E2E测试计划→E2E评审→确认）。内部使用 `todolist` 工具管理步骤
- Phase 2 使用 `/loop --max 20 继续开发需求` 启动。Stage 1 内部的 plan Task 也使用 `todolist` 管理进度
...
- `/track <需求描述>` — 启动 Phase 1（内部使用 `todolist` 工具）
...
> **向后兼容：** 如果 CLAUDE.md 使用旧格式（包含内嵌编码规范章节但无「文档索引」），输出警告但不阻塞：
> "检测到 CLAUDE.md 使用旧格式（无文档索引）。建议运行 init harness 迁移到新格式。"
> 仅当缺少「项目背景」或「质量门禁」时才阻塞。缺少「文档索引」和标准文档时警告但不阻塞。
...
> 后续自动执行阶段（Stage 9-15）需要 `/loop` 模式支持。请先输入以下命令激活循环模式：
> ` /loop --max 20 开发需求：{需求简述}`
>
> 激活后我会自动继续。

**为什么需要 /loop 模式：**
- **自动继续：** 自动化阶段 Stage 9-15 可能在长时间执行后上下文耗尽。`/loop` 模式会在每轮结束后自动重试，确保不中断
- **防卡死检测：** 连续 3 轮无进展时自动停止，防止无限循环
- **任务追踪：** loop_task_tracker 管理阶段进度，中断后可恢复
- **预算保护：** 上下文使用超过 80% 时触发收尾流程，防止丢失工作

> **注意：** Stage 1-8 可以不在 `/loop` 模式下执行（需要用户交互），但**必须在进入 Stage 9 前激活 `/loop`**。Stage 8 用户确认后会再次确认。

## 前置检查通过后

0. **读取术语表** — 读取 `references/glossary.md`，统一业务语言。后续所有阶段中的 subagent 派遣和用户交互都使用术语表中的标准用词。

1. **`create_tasks`** 注册 15 个阶段:
...
如果当前尚未在 `/loop` 模式下运行，Stage 8 用户确认后应提示用户激活 `/loop`：

> 接下来进入自动执行阶段（Stage 9-15）。建议现在输入 `/loop --max 20 继续开发需求` 激活循环模式，
> 确保后续执行不中断。
...
└── {yyyy-MM-dd}-{主题}/
    ...
    │   ├── spec_review_v1.md       # Phase 1 Step 3 (二阶段模式)
    │   ├── e2e_test_plan_review_v1.md # Phase 1 Step 7 (二阶段模式)
...
## Stage 1：需求讨论（交互阶段 — 主 agent 直接执行）
...
6. 进入 Step 4（Plan 编写）时，分阶段展开子任务：
   ...
   - 每完成一个子任务调用 `todolist complete_subtask(4, subtaskId)`
5. 执行 writing-plans skill：
   - 基于 spec.md 规划文件结构
...
| 子步骤 | Agent | 输入 | 输出 |
|--------|-------|------|------|
| A-1 后端设计 | harness-backend-planner | spec.md + plan.md 总纲 + CLAUDE.md + 项目代码 | plan-backend.md + plan-api-contract.md |
| A-2 前端设计 | harness-frontend-planner | spec.md + plan.md 总纲 + CLAUDE.md | plan-frontend.md |

A-1 和 A-2 可并行执行。harness-backend-planner 还会创建或更新 `docs/architecture.md`。

**每完成一个子步骤，调用 `todolist complete_subtask(4, subtaskId)` 标记进度。**
...
harness-api-alignment 以后端 API 合约为准，修正前端设计中的 API 调用。如果发现后端遗漏的 API，报告给主 agent 决定是否回退。

**步骤 C：汇总**

所有子文档就绪后，更新 plan.md 总纲的子文档索引（确认所有文件存在）。

**L1 不需要上述额外步骤**——所有设计都在 plan.md 单文件中。
...
plan.md 中每个 task 必须包含以下 4 个要素。缺少验收标准或文件变更表的 task 在 Stage 3 Spec 评审时标记为 MUST FIX。
...
该章节在 Stage 10 编码评审时作为数据流合规检查的依据。

#### spec.md 已做决策章节（始终必填）

spec.md 必须包含「已做决策」章节，明确列出已经确定的技术选型。AI agent 不知道决策已做出时会自行重新选择。缺少该章节的 spec 在 Stage 3 Spec 评审时标记为 MUST FIX。
...
该章节在 Stage 10 编码评审时作为技术选型合规检查的依据。

#### spec.md 行为约束章节（始终必填）

spec.md 必须包含「行为约束」章节，用三层边界约束 agent 行为。比自由文本的"约束"章节更明确，对 agent 的指导性更强。缺少该章节的 spec 在 Stage 3 Spec 评审时标记为 MUST FIX。
...
该章节在 Stage 10 编码评审时作为行为合规检查的依据。

#### spec.md 已有基础设施章节（始终必填）

spec.md 必须包含「已有基础设施」章节，让 Phase 2 编码 agent 能复用现有代码、避开已知问题。缺少该章节的 spec 在 Stage 3 Spec 评审时标记为 MUST FIX。
...
该章节在 Stage 10 编码评审时作为基础设施复用检查的依据。
...
- 失败 → 回退到 Stage 1，主 agent 直接修复

### 4. 人工确认点 1：需求待决议确认

**⚠️ 强制暂停。必须在此处等待用户回复后才能继续。绝对禁止跳过此确认点直接进入 Stage 3 Spec 评审。**
...
**流转规则：**
- 确认 → 执行 compaction（步骤 4），然后进入 Stage 3 Spec 评审（自动阶段，开始 subagent 模式）
...
| 整体评审 | harness-reviewer | xyz-harness-expert-reviewer(计划评审模式) | spec.md + plan.md + 项目根目录 |

三个评审并行执行。主 agent 收集结果后：
- 汇总所有 MUST FIX 到统一的评审报告中
- 后端评审报告写入 `changes/reviews/backend_plan_review_v{N}.md`
- 前端评审报告写入 `changes/reviews/frontend_plan_review_v{N}.md`
- 整体评审报告写入 `changes/reviews/plan_review_v{N}.md`
...
2. 读取项目文档中的架构约束和编码规范（优先 docs/standards.md + docs/architecture.md，不存在时回退读 CLAUDE.md）
...
> **进入 Stage 9 前的重要步骤：** 如果当前尚未激活 `/loop` 模式（Stage 1-8 可以不在 loop 下执行），
> 建议输入 `/loop --max 20 继续开发需求` 激活循环模式，确保后续自动化阶段在上下文耗尽后能自动继续。
>
> **二阶段模式：** 如果采用二阶段模式（Phase 1 + Phase 2 分离），则不需要激活 /loop。
> Phase 1 完成后（Step 8 用户确认），读取 `references/phase2-launch-template.md` 模板，按变量填充说明生成标准化的 Phase 2 启动提示词，让用户在新 session 中执行。
> 禁止自行精简模板或遗漏章节。
...
**重要限制：** subagent 内部不能嵌套调用 subagent。Stage 9 的编码实现由**主 agent 直接按 plan.md 的 task 逐个派遣 subagent**，而非通过一个执行 subagent 内部调度。

**主 agent 执行流程：**

1. 读取 plan.md，提取所有 task（含完整描述和上下文）
2. 使用 `todolist create_tasks` 创建 plan Task 列表（每个 task 作为一个独立条目，memoryDir 指向产出目录）
...
2. 读取编码规范（优先 docs/standards.md，不存在时回退读 CLAUDE.md 编码规范章节）
...
║   [交互] Step 2: Spec 编写 + 六要素检查 + 引用扫描          ║
║   [自动] Step 3: Spec 评审 (harness-spec-reviewer)         ║
║   [交互] Step 4: Plan 编写 (L2 时并行 planner subagent)    ║
║   [自动] Step 5: Plan 评审 (harness-reviewer + 前后端 reviewer) ║
║   [自动] Step 6: E2E 测试计划 (主agent框架 + subagent用例)   ║
║   [自动] Step 7: E2E 测试计划评审 (harness-e2e-test-plan-reviewer) ║
║   [交互] Step 8: 用户确认 ← 强制暂停                        ║
║   产出: spec.md + plan.md + e2e-test-plan.md + 评审报告      ║
╚════════════════════════════════════════════════════════════╝
    ↓ 用户 /new 创建新 session
╔════════════════════════════════════════════════════════════╗
...
| Phase 2 Stage | 单阶段模式对应 | gate-script.sh 编号 | 检查内容 |
|---------------|---------------|---------------------|----------|
| Stage 1 编码实现 | 阶段 3 编码实现 | 03 | 编译+测试+lint |
| Stage 2 编码评审 | 阶段 4 编码评审 | 02 | TDD 提交顺序检测 |
| Stage 3 单元测试 | 阶段 5 测试编写 | 05 | 新增测试文件+测试通过 |
| Stage 4 E2E 测试 | 无对应 | 无 L1 gate | harness-state.sh 直接 pass |
| Stage 5 测试评审 | 阶段 6 测试评审 | 无 L1 gate | harness-state.sh 直接 pass |
| Stage 6 推送+CI+部署 | 阶段 7+8+9 | 07→08→09 | 推送→CI→部署 三重门禁 |
| Stage 7 自动复盘 | 阶段 11 自动复盘 | 无 L1 gate | — |

> gate-script.sh 的编号基于 15 stage 设计，Phase 2 调用时必须使用对应编号，不要直接用 Phase 2 的 1-7 编号。
