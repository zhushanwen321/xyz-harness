...
"
# 用 AI 视觉对比设计稿与实际截图
python3 {zai_vision_script} ui-diff \
  "{设计稿路径}" \
...
#### 3.2 AI 视觉对比（zai-vision）

```bash
# 对比设计稿和实际截图
python3 {zai_vision_script_path} ui-diff \
  "design/{设计稿文件}.png" \
...
# 用 AI 分析拼图
python3 {zai_vision_script_path} analyze-image combined.png \
...
**AI 视觉脚本路径**：使用 `zai-vision` skill 下的 `scripts/zai_vision.py`
...
5. 每个用例按验证层级逐一检查：
   ...
   - Layer 3: CDP 截图 + zai-vision 与设计稿对比
