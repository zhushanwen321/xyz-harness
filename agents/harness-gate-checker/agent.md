...
- **失败回退：** → Stage 9 重新派遣执行 subagent（task 内修复）
...
- [ ] 新增测试通过
- [ ] `.xyz-harness/gate/stage-11.pass` 存在
- **失败回退：** 代码不可测试 → Stage 9；测试质量问题 → Stage 11 修复

### Stage 13（测试评审）
- [ ] 评审报告文件存在且非空
- [ ] 无未解决的 MUST FIX 项
- [ ] 评审轮次 ≤ 2
- **失败回退：** → Stage 11（测试质量问题）

### Stage 14（推送+CI+部署）— 代码推送
- [ ] git status --short 为空（工作区干净）
- [ ] git log origin/{branch} 有新 commit
- [ ] `.xyz-harness/gate/stage-14.pass` 存在
- **失败回退：** 修复重试（不回退阶段，网络/权限问题）

### Stage 14（推送+CI+部署）— CI 验证
- [ ] 所有本地验证命令 exit code == 0
- [ ] 测试数 > 0 且 passed == total
- [ ] `.xyz-harness/gate/stage-14.pass` 存在
- **失败回退：** 测试数=0 → Stage 11；编译错误 → Stage 9；测试失败 → Stage 9 或 11

### Stage 14（推送+CI+部署）— 部署验证
- [ ] deploy_result.md 存在且包含成功关键词（或健康检查通过）
- [ ] `.xyz-harness/gate/stage-14.pass` 存在
- **失败回退：** 代码问题 → Stage 9；配置问题 → 就地修复后重试
