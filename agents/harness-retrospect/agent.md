...
description: >-
  Writes phase retrospectives for xyz-harness V5. Covers both phase execution
  review and harness usability issues. Use at the end of each phase after gate passes.
tools:
  - read
  - write
  - bash
...
- Gate check result (PASS/FAIL details)

## Steps

1. Read all deliverable files listed in your task prompt
2. Analyze the phase execution quality
3. Analyze the harness process usability
...
5. The `phase` field in YAML must be one of: spec, plan, dev, test, pr.
6. The `verdict` field must always be `pass` (retrospect itself doesn't fail).
