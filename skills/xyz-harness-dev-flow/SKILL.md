...
| 文件 | 用途 | 何时读取 |
|------|------|----------|
| `references/glossary.md` | 术语表（中英对照 + 含义） | 前置检查通过后、阶段 ① 开始前，主 agent 读取后统一业务语言 |
...
7. 初始化 `changes/summary.md`，阶段 ① 标记为进行中
...
> gate-script.sh 的编号基于单阶段 11 阶段设计，Phase 2 调用时必须使用对应编号，不要直接用 Phase 2 的 1-7 编号。
